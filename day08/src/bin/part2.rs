...
fn process(input: &str) -> i32 {
    ...
    let nodes_to_consider = cols.max(rows);
    ...
    for freq in frequencies {
        ...
        for (a, b) in tower_positions.into_iter().tuple_combinations() {
            let a_to_b = b - a;
            let offsets = (0..nodes_to_consider).map(|i| Pos {
                row: a_to_b.row * i,
                col: a_to_b.col * i,
            });
            for node in offsets
                .flat_map(|offset| [b + offset, a - offset])
                .into_iter()
            {
...
mod tests {
    ...
    #[test]
    fn test() {
        let check = include_str!("../../../day08/check2.txt");
        assert_eq!(process(check), 34)
    }
}
