...
fn process(input: &str, cols: i32, rows: i32) -> i32 {
    ...
    for guard in guards.iter() {
        if (0..midcol).contains(&guard.pos.col) && (0..midrow).contains(&guard.pos.row) {
            safety1 += 1;
        } else if (0..midcol).contains(&guard.pos.col) && (midrow + 1..).contains(&guard.pos.row) {
            safety2 += 1;
        } else if (midcol + 1..).contains(&guard.pos.col) && (0..midrow).contains(&guard.pos.row) {
            safety3 += 1;
        } else if (midcol + 1..).contains(&guard.pos.col) && (midrow + 1..).contains(&guard.pos.row)
        {
            safety4 += 1;
        }
    }
